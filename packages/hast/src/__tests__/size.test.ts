...
describe('sizeOfHast', () => {
  test('counts text as Unicode grapheme clusters', () => {
    ...
    expect(sizeOfHast(root([]))).toBe(0);
    expect(sizeOfHast(root([text('abc'), text('中文'), text('a\nb')]))).toBe(8);
    expect(sizeOfHast(root([text('👨‍👩‍👧‍👦🇨🇳👍🏽e\u0301')]))).toBe(4);
    expect(sizeOfHast(root([text('\n')]))).toBe(1);
  });
  ...
  test('is independent of visible node order and nesting structure', () => {
    ...
    expect(sizeOfHast(flat)).toBe(3);
    expect(sizeOfHast(nested)).toBe(3);
  });
  ...
  test('treats empty text as transparent around table whitespace', () => {
    ...
    expect(sizeOfHast(root([table, text('\n')]))).toBe(1);
    expect(sizeOfHast(root([table, text(''), text('\n')]))).toBe(1);
  });
  ...
  test('handles deeply nested trees without recursive traversal', () => {
    ...
    expect(sizeOfHast(root([child]))).toBe(1);
  });
});

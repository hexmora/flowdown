...
import { createTypeOfSlot, SlotProvider, useSlots } from '..';
...
describe('slots', () => {
  ...
  test('uses tuple priorities to order slot layers', () => {
    const First = createSlotPlugin('first', 'Paragraph', CompositionSlotA);

    const Second = createSlotPlugin('second', 'Paragraph', CompositionSlotB);

    const Last = createSlotPlugin('last', 'Paragraph', CompositionSlotC);

    render(
      <SlotHarness
        plugins={[
          [Last, { priority: PluginPriority.Low }] as AnySlotPluggable,
          Second,
          [First, { priority: PluginPriority.High }] as AnySlotPluggable,
        ]}
      />,
    );

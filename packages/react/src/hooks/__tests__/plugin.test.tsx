...
describe('usePlugins', () => {
  ...
  test('changes identity only when shallow pack/default elements or their order change', () => {
    const presetA = asRemark('preset-a');
    const presetB = asRemark('preset-b');
    const extraA = asRemark('extra-a');
    const extraB = asRemark('extra-b');
    ...
    const { result, rerender } = renderHook(
      ({ defaults, packs }: { defaults: RemarkItem[]; packs: IPluginItem[] }) =>
        usePlugins(packs, 'remarks', defaults),
      {
        initialProps: {
    ...
    const reorderedPacks = result.current;

    expect(reorderedPacks).not.toBe(initial);
    expect(reorderedPacks).toEqual([presetA, presetB, extraB, extraA]);
    ...
    expect(result.current).not.toBe(reorderedPacks);

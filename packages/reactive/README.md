...
`reactive` powers [Flowdown](../../README.md)'s core and can also be used independently for application state and data processing.
...
```ts
import { type IReadableClosure, once, useMap } from "reactive";
...
```ts
import { BaseStateClosure, type IReadableClosure } from "reactive";
...
JSX can describe the same computations as `S`. For TypeScript, set `jsx` to `"react-jsx"` and `jsxImportSource` to `"reactive"` in your compiler options, then use a `.tsx` file:

```tsx
import { MutableState, render } from "reactive";
...
This JSX describes reactive computations using `reactive`'s own runtime. It does not require React.

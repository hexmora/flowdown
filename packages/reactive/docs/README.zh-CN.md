...
`reactive` 为 [Flowdown](../../../docs/README.zh-CN.md) 的核心提供支持，也可以独立用于应用状态管理和数据处理。
...
JSX 可以描述与 `S` 相同的计算。使用 TypeScript 时，在编译选项中将 `jsx` 设为 `"react-jsx"`，将 `jsxImportSource` 设为 `"reactive"`，然后使用 `.tsx` 文件：
...
这里的 JSX 使用 `reactive` 自身的运行时来描述响应式计算，无需 React。

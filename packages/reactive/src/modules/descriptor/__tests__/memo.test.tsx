...
describe('memo', () => {
  test('creates a lazy mapped state closure', () => {
    ...
    const closure = buildDescriptor<number>(S<number>(<Doubled value={source} />));
  ...
  test('uses shallow output equality by default', () => {
    ...
    const closure = buildDescriptor<number[]>(S<number[]>(<Values value={source} />));
  ...
  test('distinguishes unequal primitive outputs', () => {
    ...
    const closure = buildDescriptor<number>(S<number>(<Identity value={source} />));
  ...
  test('accepts a custom output distinctor', () => {
    ...
    const closure = buildDescriptor<{ id: number; value: string }>(
  ...
  test('preserves D-wrapped mapper props', () => {
    ...
    const closure = buildDescriptor<string>(
  ...
  test('supports direct JSX runtime calls', () => {
    ...
    expectTypeOf(buildDescriptor(direct)).toEqualTypeOf<IStateClosure<number>>();

    expectTypeOf(buildDescriptor(nullDescriptor)).toEqualTypeOf<IStateClosure<null>>();

    expectTypeOf(buildDescriptor(undefinedDescriptor)).toEqualTypeOf<IStateClosure<undefined>>();

    const directClosure = buildDescriptor(direct);

    const nullClosure = buildDescriptor(nullDescriptor);
...
const _typecheckMemoJSX = () => {
  ...
  expectTypeOf(buildDescriptor(descriptor)).toEqualTypeOf<IStateClosure<string>>();
  ...
  // @ts-expect-error Ordinary functions are not memoized state mapper components.
  <PlainMapper value={1} />;

...
describe('switchMapClosure', () => {
  test.each(['hook', 'member'] as const)(
    ...
    (api) => {
      ...
      const mapper = vi.fn((index: number) => S([Child, { index: D(index) }]));
      ...
      expect(mapper).toHaveBeenCalledExactlyOnceWith(0);

      source.next(1);

      expect(output.value.value).toBe(2);

      expect(cleaned).toHaveBeenCalledExactlyOnceWith(0);
  ...
  test.each(['hook', 'member'] as const)(
    ...
    (api) => {
      ...
      const mapper = vi.fn((value: number) => [D(value % 2), D(2)]);
      ...
      expect(next).toHaveBeenCalledExactlyOnceWith([1, 2]);
      ...
      expect(next).toHaveBeenCalledExactlyOnceWith([0, 2]);
  ...
  test('builds lazily and does not repeat the initial mapping on subscription', () => {
    ...
    const mapper = vi.fn((value: number) => D(value * 2));
    ...
    expect(mapper).toHaveBeenCalledExactlyOnceWith(1);
  ...
  test('settles source replacement and pending inner changes in one batched output', () => {
    ...
    expect(next).toHaveBeenCalledExactlyOnceWith(4);
  ...
  test('waits for newly selected derived dependencies before notifying downstream consumers', () => {
    ...
    expect(next).toHaveBeenCalledExactlyOnceWith(5);
  ...
  test('supports result comparison across replacements', () => {
    ...
    expect(next).toHaveBeenCalledExactlyOnceWith({ parity: 0 });

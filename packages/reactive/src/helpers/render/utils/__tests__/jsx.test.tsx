...
function FunctionalFactoryStateClosure({ create }: Factory) {
  return { create };
}
...
describe('descriptor JSX runtime', () => {
  ...
  test('supports native void-input state closures', () => {
    const closure = render<'empty'>(<EmptyStateClosure />);
  ...
  test('treats an omitted default object as empty JSX props', () => {
    const closure = render<string>(<DefaultObjectStateClosure />);
...
const _typecheckDescriptorJSX = () => {
  ...
  expectTypeOf(render(descriptor)).toEqualTypeOf<IStateClosure<number>>();
  expectTypeOf(render(<CountStateClosure source={source} />)).toMatchTypeOf<
    IStateClosure<unknown>
  >();
  expectTypeOf(render<number>(<CountStateClosure source={source} />)).toEqualTypeOf<
    IStateClosure<number>
  >();

  // Raw JSX has already erased its creator, so explicit value types are trusted.
  expectTypeOf(render<string>(<CountStateClosure source={source} />)).toEqualTypeOf<
    IStateClosure<string>
  >();
  expectTypeOf(render(EmptyStateClosure)).toEqualTypeOf<EmptyStateClosure>();
  expectTypeOf(render(S([CountStateClosure, { source }]))).toEqualTypeOf<CountStateClosure>();

  const directDescriptor = jsx(CountStateClosure, { source });

  expectTypeOf(render(directDescriptor)).toEqualTypeOf<CountStateClosure>();

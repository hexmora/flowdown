...
import {
  ...
  type MarkedStateClosureDescriptor,
  MutableState,
  ReactiveState,
  render,
  S,
...
type SecondStateClosureInputs = {
  config: IReactiveState<'config'>;
};

class SecondStateClosure extends BaseStateClosure<'first', SecondStateClosureInputs> {
  static readonly instances: SecondStateClosure[] = [];

  constructor(inputs: SecondStateClosureInputs) {
    super(inputs);
...
type ForthStateClosureInputs = {
  test: IReactiveState<'forth-param'>;
};

class ForthStateClosure extends BaseStateClosure<'forth', ForthStateClosureInputs> {
  static readonly instances: ForthStateClosure[] = [];

  constructor(inputs: ForthStateClosureInputs) {
    super(inputs);
...
type FifthStateClosureInputs = {
  test: IReactiveState<'fifth-sub'>;
};

class FifthStateClosure extends BaseStateClosure<'fifth', FifthStateClosureInputs> {
  static readonly instances: FifthStateClosure[] = [];

  constructor(inputs: FifthStateClosureInputs) {
    super(inputs);
...
type SixthStateClosureInputs = {
  test1: IReactiveState<'sixth-param-1'>;
  test2: IReactiveState<'sixth-param-2'>;
};

class SixthStateClosure extends BaseStateClosure<'sixth', SixthStateClosureInputs> {
  static readonly instances: SixthStateClosure[] = [];

  constructor(inputs: SixthStateClosureInputs) {
    super(inputs);
...
type SeventhSubClosureInputs = {
  test2: IReactiveState<'seventh-sub-param'>;
};

class SeventhSubClosure extends BaseStateClosure<'seventh-sub', SeventhSubClosureInputs> {
  static readonly instances: SeventhSubClosure[] = [];

  constructor(inputs: SeventhSubClosureInputs) {
    super(inputs);
...
type SeventhStateClosureInputs = {
  test1: IReactiveState<'seventh-param'>;
  test2: (subParamState: IReactiveState<'seventh-sub-param'>) => IReactiveState<'seventh-sub'>;
};

class SeventhStateClosure extends BaseStateClosure<'seventh', SeventhStateClosureInputs> {
  static readonly instances: SeventhStateClosure[] = [];

  constructor(inputs: SeventhStateClosureInputs) {
    super(inputs);
...
class FirstStateClosure extends BaseStateClosure<'root', FirstStateClosureInputs> {
  protected render() {
...
type AdditionalStateClosureInputs = {
...
class AdditionalStateClosure extends BaseStateClosure<null, AdditionalStateClosureInputs> {
  protected render() {
    return null;
  }
}

type LifecycleStateClosureInputs = {
  name: string;
};

class LifecycleStateClosure extends BaseStateClosure<string, LifecycleStateClosureInputs> {
  static readonly destroyedNames: string[] = [];

  static readonly instances: LifecycleStateClosure[] = [];

  constructor(inputs: LifecycleStateClosureInputs) {
    super(inputs);
...
type LifecycleRootStateClosureInputs = {
  external?: IReactiveState<string>;
  first: IReactiveState<string>;
  second: IReactiveState<string>;
};

class LifecycleRootStateClosure extends BaseStateClosure<null, LifecycleRootStateClosureInputs> {
  protected render() {
    return null;
  }
}

type GeneratorRootStateClosureInputs = {
  getChild(): IReactiveState<string>;
};

class GeneratorRootStateClosure extends BaseStateClosure<null, GeneratorRootStateClosureInputs> {
  protected render() {
    return null;
  }
}

type PairGeneratorRootStateClosureInputs = {
  create(params: {
    first: IReactiveState<string>;
    second: IReactiveState<string>;
...
class PairGeneratorRootStateClosure extends BaseStateClosure<
  null,
  PairGeneratorRootStateClosureInputs
> {
  protected render() {
    return null;
  }
...
describe('descriptor', () => {
  test('keeps S as an identity and builds static descriptor nodes', () => {
    ...
    const existing = render(secondState);
    const raw = render(ConfigStateClosure);
  ...
  test('supports positional generators and immediate function values', () => {
    const closure = render(additionalDescriptor);
  ...
  test.each([new Date(0), new Map(), new ConstantClass()])(
    ...
    (constant) => {
      ...
      const closure = render(S([ImmediateValueStateClosure, D(constant)]));
  ...
  test('accepts direct constructor, mapping, and generator results', () => {
    ...
    const array = render(S([ImmediateArrayStateClosure, [1, 2, 3]]));

    const mapped = render(S([({ value }: { value: number }) => value * 2, { value: 2 }]));
...
type StateSourceStateClosureInputs = {
  create(): StateSource<number>;
  forward(source: StateSource<number>): StateSource<number>;
  source: StateSource<number>;
};

class StateSourceStateClosure extends BaseStateClosure<null, StateSourceStateClosureInputs> {
  protected render() {
    return null;
  }
}
...
const _typecheckMappedDescriptors = () => {
  ...
  const mapped = S([({ value }: { value: number }) => value * 2, { value: source }]);

  const [mappedFactory, mappedParams] = mapped;

  const [comparedFactory, comparedParams, comparedDistinctor] = compared;

  expectTypeOf(mappedFactory).toEqualTypeOf<({ value }: { value: number }) => number>();
  expectTypeOf(mappedParams.value).toEqualTypeOf<typeof source>();
  expectTypeOf<MappingDescriptorValue<typeof mapped>>().toEqualTypeOf<number>();
  expectTypeOf(mapped).toMatchTypeOf<MarkedStateClosureDescriptor<number>>();

  expectTypeOf(comparedFactory).toEqualTypeOf<
    ({ value }: { value: number }) => { value: number }
  >();
  expectTypeOf(comparedParams.value).toEqualTypeOf<typeof source>();
  expectTypeOf(comparedDistinctor).toMatchTypeOf<
    (left: { value: number }, right: { value: number }) => boolean
  >();
  expectTypeOf<MappingDescriptorValue<typeof compared>>().toEqualTypeOf<{ value: number }>();
  expectTypeOf(compared).toMatchTypeOf<MarkedStateClosureDescriptor<{ value: number }>>();
